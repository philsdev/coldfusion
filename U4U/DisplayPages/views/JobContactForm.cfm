...
<div id="content" class="popup">
    ...
    <p><small>*Please note that unless specified by the company, this contact form should only be used to send questions and comments.  Please avoid sending the full contents of a resume or cover letter.</small></p>
    
	<section class="formSection green">
		<cfif len(variables.Message)>
			<p class="message"><cfoutput>#variables.Message#</cfoutput></p>
		</cfif>
		<form action="/job-contact-submit.html" method="post" id="ContactJobForm">
			<input type="hidden" name="JobID" value="<cfoutput>#Event.GetArg('JobID')#</cfoutput>" />
