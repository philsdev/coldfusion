	...
	<cffunction name="GetAdmins" returntype="query">
		...
		<cfquery datasource="#request.dsource#" name="loc_Admins">
			...
			FROM  (
						...
						FROM			AMP_SiteAdmins A	
						JOIN			AMP_Status S ON A.Status = S.ID
						LEFT JOIN		AMP_SiteAdminPermissions P ON A.ID = P.AdminID
						LEFT JOIN		(SELECT COUNT(*) AS Total FROM AMP_SiteAdminSections) SS ON 1=1
		...
		<cfif Len( Arguments.Section )>
			<cfquery datasource="#request.dsource#" name="loc_InsertSections">
				<cfloop list="#Arguments.Section#" index="loc_thisSection">
					INSERT INTO AMP_SiteAdminPermissions (
						AdminID,
						SectionID
					) VALUES (
