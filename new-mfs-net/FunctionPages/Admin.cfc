<cfcomponent hint="This component will handle loggin in and logging out the site administrator" extends="MachII.framework.Listener">
	...
	<cfscript>
		...
		this.SortOptionsArray = ArrayNew(2);
		this.SortOptionsArray[1][1] = "asc";
		this.SortOptionsArray[1][2] = "Ascending";
		this.SortOptionsArray[2][1] = "desc";
		this.SortOptionsArray[2][2] = "Descending";
	...
	<cffunction name="ValidateAdministrator" returntype="void" hint="I check to make sure the admin is still logged in">
		...
		<cfif not StructKeyExists(REQUEST.SessionStruct, "AdminID") OR StructCount(REQUEST.SessionStruct) EQ 0>
	...
	<cffunction name="GetAdminSections" access="public" returntype="query" output="no">
		...
		<cfquery name="loc_AdminSections" datasource="#request.dsource#">
			...
			FROM			SiteAdminPermissions P
			JOIN			SiteAdminSections S ON P.SectionID = S.SectionID AND S.Active = 1
			JOIN			SiteAdminSectionGroups G ON S.SectionGroupID = G.GroupID AND G.Active = 1
	...
	<cffunction name="GetAdminPermissions" returntype="query" hint="I return the selected admin permissions">
		...
		<cfquery datasource="#request.dsource#" name="loc_AdminPermissions">
			...
			FROM			SiteAdminSectionGroups G
			JOIN			SiteAdminSections S ON G.GroupID = S.SectionGroupID AND S.Active = 1     
			LEFT JOIN		SiteAdminPermissions P ON S.SectionID = P.SectionID 
	...
	<cffunction name="GetFormattedDescription" access="public" output="false" returntype="string">
		...
		<cfset loc_Description = REReplaceNoCase( loc_Description, '<[^>]+>', '', 'all' ) />
	...
	<cffunction name="GetGridRows" access="public" output="false" returntype="struct">
		...
		<cfargument name="rows" type="numeric" default="#REQUEST.Settings.RecordsPerPage#" />
	...
	<cffunction name="GetGridPagination" access="public" output="true" returntype="string">
	...
	<cffunction name="GetListingPagination" access="public" output="false" returntype="string" hint="FRONT-END">
		<cfargument name="TotalsStruct" type="struct" required="yes" />
		<cfargument name="PaginationIdentifier" type="string" required="no" default="" />
		...
		<cfif Arguments.TotalsStruct.Total GT 1>	
			...
			<cfsavecontent variable="loc_Pagination">
				<cfoutput>
					<div class="wp-pagenavi fr" <cfif LEN(Arguments.PaginationIdentifier)>id="#Arguments.PaginationIdentifier#"</cfif>>
						<span class="pages">Page #NumberFormat(loc_Page)# of #NumberFormat(loc_Total)#</span>
						<cfif loc_Page GT 1>
							<a class="nextpostslink" p="1" title="First">First</a>
							<a class="nextpostslink" p="#INT( loc_Page - 1 )#" title="Previous">&laquo;</a>
						</cfif>
						<cfloop list="#loc_PageList#" index="loc_PageIndex">
							...
							<cfelse>
								<a class="page" p="#loc_PageIndex#">#NumberFormat(loc_PageIndex)#</a>
							</cfif>
						</cfloop>
						<cfif loc_Page LT loc_Total>
							<a class="nextpostslink" p="#INT( loc_Page + 1 )#" title="Next">&raquo;</a>
							<a class="nextpostslink" p="#loc_Total#" title="Next">Last</a>
						</cfif>
					</div>
	...
	<cffunction name="GetSubmitSubSectionButtons" access="public" returntype="string" output="no">
		<cfargument name="UrlStruct" required="yes" type="struct" />
		...
		<cfsavecontent variable="loc_Buttons">
			<cfoutput>
				<div class="submitButtonContainer">
					<button type="button" class="cancelSubSection">Cancel</button>
					<cfif NOT StructKeyExists( Arguments.UrlStruct, "Type" ) 
						OR ( StructKeyExists( Arguments.UrlStruct, "Type" ) AND Arguments.UrlStruct["Type"] NEQ "view" )>
						<button type="submit" class="stay">Save and Keep Editing</button>
						<button type="submit" class="continue">Save and Return to List</button>
	...
	<cffunction name="GetSelectBox" access="public" returntype="string" output="yes">
		...
		<cfargument name="OmitWhenNoRecordset" required="no" type="any" default="false" />
		<cfargument name="CssClass" default="" />
		...
		<cfsavecontent variable="loc_Box">		
			<cfif NOT Arguments.OmitWhenNoRecordset OR ( Arguments.OmitWhenNoRecordset AND Arguments.DataQuery.RecordCount )>
				<select 
					...
					<cfif LEN(Arguments.CssClass)>class="#Arguments.CssClass#"</cfif>
				>
					...
					<cfoutput query="Arguments.DataQuery">
						...
						<cfset loc_thisLabel = loc_LinkManager.GetDisplayName( loc_thisLabel ) />
	...
	<cffunction name="GetSortDirectionBox" access="public" returntype="string" output="no">
		<cfargument name="Sord" required="no" default="" type="any" />
		...
		<cfsavecontent variable="loc_Box">			
			<cfoutput>
			<select name="Sord" id="Sord">
				<cfif LEN( Arguments.FirstLabel )>
					<option value="">#Arguments.FirstLabel#</option>
				</cfif>
				<cfloop from="1" to="#ArrayLen(this.SortOptionsArray)#" index="loc_thisIndex">
					<cfset loc_ThisValue = this.SortOptionsArray[loc_thisIndex][1] />
					<cfset loc_ThisLabel = this.SortOptionsArray[loc_thisIndex][2] />
					<option value="#loc_ThisValue#" <CFIF Arguments.Sord EQ loc_ThisValue>selected="selected"</CFIF>>#loc_ThisLabel#</option>
				</cfloop>
			</select>
			</cfoutput>
		</cfsavecontent>
	...
	<cffunction name="GetSortOrder" access="public" returntype="string" output="no">
		<cfargument name="DefaultSord" type="string" required="yes" />
		<cfargument name="Sord" type="string" required="no" default="" />
		
		<cfset var loc_SortOrder = "" />
		
		<cfif StructKeyExists( FORM, "Sord" )>
			<cfset loc_SortColumn = FORM.Sord />
		<cfelse>	
			<cfset loc_SortOrder = Arguments.DefaultSord />
		</cfif>
		
		<cfreturn loc_SortOrder />
	</cffunction>
	...
	<cffunction name="SendContactForm" access="public" output="true" returntype="void">    
		...
		<cftry>
			<cfmail		to="#REQUEST.Email.Admin#"
						from="#REQUEST.Email.Sender#"
						subject="User Contact Request from #Request.WebDisplay#">You have received the following contact request:
